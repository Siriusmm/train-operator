...
func newDeepspeedJobCommon(name string, startTime, completionTime *metav1.Time) *kubeflowv1.DeepspeedJob {
	DeepspeedJob := &kubeflowv1.DeepspeedJob{
		...
		Spec: kubeflowv1.DeepspeedJobSpec{
			...
			DeepspeedReplicaSpecs: map[common.ReplicaType]*common.ReplicaSpec{
				kubeflowv1.DeepspeedJobReplicaTypeWorker: {
				...
				kubeflowv1.DeepspeedJobReplicaTypeLauncher: {
					Template: corev1.PodTemplateSpec{
						Spec: corev1.PodSpec{
							Containers: []corev1.Container{
								{
...
func newDeepspeedJobOld(name string, replicas *int32, pusPerReplica int64, resourceName string, startTime, completionTime *metav1.Time) *kubeflowv1.DeepspeedJob {
	...
	DeepspeedJob.Spec.DeepspeedReplicaSpecs[kubeflowv1.DeepspeedJobReplicaTypeWorker].Replicas = replicas

	workerContainers := DeepspeedJob.Spec.DeepspeedReplicaSpecs[kubeflowv1.DeepspeedJobReplicaTypeWorker].Template.Spec.Containers
...
func newDeepspeedJobWithLauncher(name string, replicas *int32, pusPerReplica int64, resourceName string, startTime, completionTime *metav1.Time) *kubeflowv1.DeepspeedJob {
	...
	DeepspeedJob.Spec.DeepspeedReplicaSpecs[kubeflowv1.DeepspeedJobReplicaTypeLauncher].Replicas = ptr.To[int32](1)

	launcherContainers := DeepspeedJob.Spec.DeepspeedReplicaSpecs[kubeflowv1.DeepspeedJobReplicaTypeLauncher].Template.Spec.Containers
...
var _ = Describe("DeepspeedJob controller", func() {
	Context("Test launcher is GPU launcher", func() {
		It("Should pass GPU Launcher verification", func() {
			By("By creating DeepspeedJobs with various resource configuration")
			...
			for testName, testCase := range testCases {
				DeepspeedJob := newDeepspeedJobWithLauncher("test-"+strings.ToLower(testName),
					ptr.To[int32](64), 1, testCase.gpu, &startTime, &completionTime)
				Expect(isGPULauncher(DeepspeedJob) == testCase.expected).To(BeTrue())
			}
		})
	})
	...
	Context("Test DeepspeedJob with succeeded launcher Pod", func() {
		It("Should contains desired launcher ReplicaStatus", func() {
			...
			DeepspeedJob := newDeepspeedJobWithLauncher(jobName, ptr.To[int32](64), 1, gpuResourceName, &startTime, &completionTime)
			Expect(testK8sClient.Create(ctx, DeepspeedJob)).Should(Succeed())

			launcher := reconciler.newLauncher(DeepspeedJob, "kubectl-delivery", isGPULauncher(DeepspeedJob))
			...
			Eventually(func() bool {
				...
				return ReplicaStatusMatch(created.Status.ReplicaStatuses, kubeflowv1.DeepspeedJobReplicaTypeLauncher, launcherStatus)
	...
	Context("Test DeepspeedJob with succeeded launcher pod", func() {
		It("Should contain desired ReplicaStatuses for worker", func() {
			...
			Eventually(func() bool {
				...
				return ReplicaStatusMatch(created.Status.ReplicaStatuses, kubeflowv1.DeepspeedJobReplicaTypeWorker, launcherStatus)
			}, testutil.Timeout, testutil.Interval).Should(BeTrue())
		})
	})
	...
	Context("Test DeepspeedJob with Running launcher Pod and Pending worker Pods", func() {
		It("Should contain desired ReplicaStatuses", func() {
			...
			for i := 0; i < int(replicas); i++ {
				name := fmt.Sprintf("%s-%d", DeepspeedJob.Name+workerSuffix, i)
				worker := reconciler.newWorker(DeepspeedJob, name)
			...
			Eventually(func() bool {
				...
				return ReplicaStatusMatch(created.Status.ReplicaStatuses, kubeflowv1.DeepspeedJobReplicaTypeLauncher,
					launcherStatus) && ReplicaStatusMatch(created.Status.ReplicaStatuses, kubeflowv1.DeepspeedJobReplicaTypeWorker,
					workerStatus)
			}, testutil.Timeout, testutil.Interval).Should(BeTrue())
		})
	})
	...
	Context("Deepspeed Job succeeds with predefined service account", func() {
		It("should run with the defined service account", func() {
			...
			DeepspeedJob := newDeepspeedJob(jobName, ptr.To[int32](64), 1, gpuResourceName, &startTime, &completionTime)
			DeepspeedJob.Spec.DeepspeedReplicaSpecs[kubeflowv1.DeepspeedJobReplicaTypeLauncher].Template.Spec.ServiceAccountName = launcherSaName
			sa := newLauncherServiceAccount(DeepspeedJob)
	...
	Context("DeepspeedJob with launcher Pod not controlled by itself", func() {
	...
	Context("DeepspeedJob with worker Pod not controlled by itself", func() {
		It("Should return error", func() {
			...
			DeepspeedJob := newDeepspeedJob(jobName, ptr.To[int32](1), 1, gpuResourceName, &startTime, &completionTime)
	...
	Context("DeepspeedJob with ConfigMap not controlled by itself", func() {
		It("Should return error", func() {
			...
			cm := newConfigMap(DeepspeedJob, 64, isGPULauncher(DeepspeedJob))
	...
	Context("DeepspeedJob with Role not controlled by itself", func() {
		It("Should return error", func() {
			By("Calling Reconcile method")
			...
			role := newLauncherRole(DeepspeedJob, 64)
	...
	Context("DeepspeedJob with RoleBinding not controlled by itself", func() {
		It("Should return error", func() {
			By("Calling Reconcile method")
			...
			rb := newLauncherRoleBinding(DeepspeedJob)
	...
	Context("Test launcher's Intel Deepspeed handling", func() {
		It("Should create a launcher job with Intel Deepspeed env variables", func() {
			By("By creating DeepspeedJobs with and without preset env variables")

			testCases := map[string]struct {
				envVariables         map[string]string
				expectedEnvVariables map[string]string
			}{
				"withoutIDeepspeedValues": {
					envVariables: map[string]string{
						"X_Deepspeed_HYDRA_BOOTSTRAP": "foo",
					},
					expectedEnvVariables: map[string]string{
						"I_Deepspeed_HYDRA_BOOTSTRAP":      iDeepspeedDefaultBootstrap,
						"I_Deepspeed_HYDRA_BOOTSTRAP_EXEC": fmt.Sprintf("%s/%s", configMountPath, kubexecScriptName),
					},
				},
				"withIDeepspeedBootstrap": {
					envVariables: map[string]string{
						"I_Deepspeed_HYDRA_BOOTSTRAP": "RSH",
					},
					expectedEnvVariables: map[string]string{
						"I_Deepspeed_HYDRA_BOOTSTRAP":      "RSH",
						"I_Deepspeed_HYDRA_BOOTSTRAP_EXEC": fmt.Sprintf("%s/%s", configMountPath, kubexecScriptName),
				...
				"withIDeepspeedBootstrapExec": {
					envVariables: map[string]string{
						"I_Deepspeed_HYDRA_BOOTSTRAP_EXEC": "/script.sh",
					},
					expectedEnvVariables: map[string]string{
						"I_Deepspeed_HYDRA_BOOTSTRAP":      iDeepspeedDefaultBootstrap,
						"I_Deepspeed_HYDRA_BOOTSTRAP_EXEC": "/script.sh",
				...
				"withIDeepspeedBootstrapAndExec": {
					envVariables: map[string]string{
						"I_Deepspeed_HYDRA_BOOTSTRAP":      "RSH",
						"I_Deepspeed_HYDRA_BOOTSTRAP_EXEC": "/script.sh",
					},
			...
			for testName, testCase := range testCases {
				...
				template := &DeepspeedJob.Spec.DeepspeedReplicaSpecs[kubeflowv1.DeepspeedJobReplicaTypeLauncher].Template
				...
				launcher := reconciler.newLauncher(DeepspeedJob, "kubectl-delivery", false)
	...
	Context("When creating the DeepspeedJob with the suspend semantics", func() {
		const name = "test-job"
		var (
			ns          *corev1.Namespace
		...
		BeforeEach(func() {
			ns = &corev1.Namespace{
				ObjectMeta: metav1.ObjectMeta{
					GenerateName: "Deepspeedjob-test-",
				},
			}
			Expect(testK8sClient.Create(ctx, ns)).Should(Succeed())

			now := metav1.Now()
			job = newDeepspeedJob(name, ptr.To[int32](1), 1, gpuResourceName, &now, &now)
		...
		It("Shouldn't create resources if DeepspeedJob is suspended", func() {
			By("By creating a new DeepspeedJob with suspend=true")
			...
			By("Checking created DeepspeedJob")
			Eventually(func() bool {
				err := testK8sClient.Get(ctx, jobKey, created)
				return err == nil
			}, testutil.Timeout, testutil.Interval).Should(BeTrue())
			By("Checking created DeepspeedJob has a nil startTime")
			...
			By("Checking if the DeepspeedJob has suspended condition")
			Eventually(func() []kubeflowv1.JobCondition {
			...
			}, testutil.ConsistentDuration, testutil.Interval).Should(BeComparableTo([]kubeflowv1.JobCondition{
				{
					...
					Reason:  commonutil.NewReason(kubeflowv1.DeepspeedJobKind, commonutil.JobCreatedReason),
					Message: fmt.Sprintf("DeepspeedJob %s is created.", name),
				},
				{
					Type:    kubeflowv1.JobSuspended,
					Status:  corev1.ConditionTrue,
					Reason:  commonutil.NewReason(kubeflowv1.DeepspeedJobKind, commonutil.JobSuspendedReason),
					Message: fmt.Sprintf("DeepspeedJob %s is suspended.", name),
				},
			}, testutil.IgnoreJobConditionsTimes))
		})

		It("Should delete resources after DeepspeedJob is suspended; Should resume DeepspeedJob after DeepspeedJob is unsuspended", func() {
			By("By creating a new DeepspeedJob")
			Expect(testK8sClient.Create(ctx, job)).Should(Succeed())
			...
			// We'll need to retry getting this newly created DeepspeedJob, given that creation may not immediately happen.
			By("Checking created DeepspeedJob")
			Eventually(func() bool {
				err := testK8sClient.Get(ctx, jobKey, created)
				return err == nil
			...
			By("Checking the DeepspeedJob's condition")
			Eventually(func() []kubeflowv1.JobCondition {
				Expect(testK8sClient.Get(ctx, jobKey, created)).Should(Succeed())
				return created.Status.Conditions
				...
				},
				{
					...
					Status:  corev1.ConditionTrue,
					Reason:  commonutil.NewReason(kubeflowv1.DeepspeedJobKind, commonutil.JobRunningReason),
					Message: fmt.Sprintf("DeepspeedJob %s is running.", name),
				},
			}, testutil.IgnoreJobConditionsTimes))

			By("Updating the DeepspeedJob with suspend=true")
			Eventually(func() error {
				Expect(testK8sClient.Get(ctx, jobKey, created)).Should(Succeed())
			...
			By("Checking if the DeepspeedJob has a suspended condition")
			Eventually(func() bool {
				Expect(testK8sClient.Get(ctx, jobKey, created)).Should(Succeed())
				return created.Status.ReplicaStatuses[kubeflowv1.DeepspeedJobReplicaTypeLauncher].Active == 0 &&
					created.Status.ReplicaStatuses[kubeflowv1.DeepspeedJobReplicaTypeWorker].Active == 0 &&
			...
			By("Unsuspending the DeepspeedJob")
			Eventually(func() error {
				Expect(testK8sClient.Get(ctx, jobKey, created)).Should(Succeed())
			...
			By("Checking if the DeepspeedJob has resumed conditions")
			Eventually(func() []kubeflowv1.JobCondition {
				Expect(testK8sClient.Get(ctx, jobKey, created)).Should(Succeed())
				return created.Status.Conditions
			}, testutil.Timeout, testutil.Interval).Should(BeComparableTo([]kubeflowv1.JobCondition{
				...
				{
					Type:    kubeflowv1.JobSuspended,
					Reason:  commonutil.NewReason(kubeflowv1.DeepspeedJobKind, commonutil.JobResumedReason),
					Message: fmt.Sprintf("DeepspeedJob %s is resumed.", name),
					Status:  corev1.ConditionFalse,
				},
				{
					Type:    kubeflowv1.JobRunning,
